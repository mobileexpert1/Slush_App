...
class reelController with ChangeNotifier{
  ...
  Future swippedVideo(BuildContext context,int id)async{
    ...
    if(response.statusCode==201){print("response.statusCode===201=ApiList.swippedVideo");// getVideoCount(context);
    }
    else if(response.statusCode==401){showToastMsgTokenExpired();}
    else{}
  }
  ...
  Future getVidero(BuildContext context,int pageI,int minage,int maxage,int distance,String lat,String lon,String gender,{bool filter=false})async{
    ...
    } else{total=-1;dataa=ii;}notifyListeners();}

  Future loadmore(BuildContext context,int i,int minage,int maxage,int distance,String lat,String lon,String gender)async{
    pages=pages+1;
    final reelcntrol=Provider.of<reelController>(context,listen: false);
    final url="${ApiList.getVideo}$minage&maxAge=$maxage&distance=$distance&latitude=$lat&longitude=$lon$genderparam&isVerified=${LocaleHandler.isChecked}&page=$pages&limit=5";
    print(url);
    var uri=Uri.parse(url);
  ...
  void playNextReel(int index) {
    ...
    if(index>0){_stopControllerAtIndex(index - 1);}
    /// Dispose [index - 2,3] controller
    if(index>1){ _disposeControllerAtIndex(index - 2);}
  ...
  void playPreviousReel(int index) {
    ...
    playControllerAtIndex(index);
    /// Initialize [index - 1] controller
    if(index>0){_initializeControllerAtIndex(index - 1);}
    LocaleHandler.pageIndex=index;
    notifyListeners();
  }
  ...
  void _stopControllerAtIndex(int index) {
    if (videoPlayerController.length > index && index >= 0) {
      ...
      for(var i=0;i<videoPlayerController.length;i++){videoPlayerController[i].pause();}
  ...
  void playControllerAtIndex(int index) {
    ...
    if (videoPlayerController.length > index && index >= 0) {
      ...
      if (controller.value.isInitialized) {
        /// Play controller
        //  _chewieController = ChewieController(videoPlayerController: controller, autoPlay: true, looping: true,);
        //  controller.play();
        print("play2;-;-;-;-");
        ...
        if(index==0){controller.pause();}
          ...
          controller.play();
        controller.setLooping(true);}
        if (controller.value.isPlaying) { controller.setVolume(isMuted?0.0:1.0);} else {controller.setVolume(0.0);}
      } else {
        _initializeControllerAtIndex(index).then((value) {
          if(index==0){videoPlayerController[index].pause();}
          else {
            played=true;
            videoPlayerController[index].play();
          videoPlayerController[index].setLooping(true);}
          if(videoPlayerController[index].value.isPlaying){ videoPlayerController[index].setVolume(isMuted?0.0:1.0);}
          else{videoPlayerController[index].setVolume(0.0);}
