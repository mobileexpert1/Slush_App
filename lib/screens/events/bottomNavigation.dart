...
class _BottomNavigationScreenState extends State<BottomNavigationScreen> {
  ...
  void _onItemTapped(int index) {
    ...
    if (LocaleHandler.matchedd || LocaleHandler.bioAuth) {}
      ...
      } else if (_selectedIndex.value != 0) {Provider.of<eventController>(context, listen: false).timerCancel();}
    }
      ...
      child: Stack(
        children: [
                          ...
                          items: <BottomNavigationBarItem>[
                            BottomNavigationBarItem(
                                icon: SvgPicture.asset(
                                    _selectedIndex.value == 0 ? AssetsPics.selectedevnetIcon : AssetsPics.evnetIcon),
                                label: 'Event',
                                backgroundColor: color.txtBlue),
                            BottomNavigationBarItem(
                                icon: Stack(
                                  children: [
                                    SvgPicture.asset(_selectedIndex.value == 1 &&
                                        Provider.of<ChatController>(context, listen: false).unreadMsg
                                        ? AssetsPics.selctedredmessage
                            ...
                            BottomNavigationBarItem(
                              icon: SvgPicture.asset(
                                  _selectedIndex.value == 2 ? AssetsPics.selectedreelIcon : AssetsPics.reelIcon),
                              label: 'Reel',
                            ),
                            BottomNavigationBarItem(
                              icon: SvgPicture.asset(
                            ...
                            BottomNavigationBarItem(
                              icon: SvgPicture.asset(
                                  _selectedIndex.value == 4
                                      ? AssetsPics.selectedprofileIcon
                                      : AssetsPics.profileIcon),
                              label: 'Profile',
                            ),
                          ],
          ...
          Consumer<profileController>(builder: (context, val, child) {
            return val.unmatched
                ? CustomTopToaster(textt: "Unmatched successfully")
