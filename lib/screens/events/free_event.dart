...
class _EvenetFreeScreenState extends State<EvenetFreeScreen> {
                    ...
                    GestureDetector(
                      onTap: () {
                        ...
                        if (LocaleHandler.items.contains(data["eventId"])) {
                          eventCntrller.saveEvent(
                              ApiList.unsaveEvent, data["eventId"].toString());
                          LocaleHandler.items.remove(data["eventId"]);
                        } else {
                          eventCntrller.saveEvent(
                              ApiList.saveEvent, data["eventId"].toString());
  ...
  Widget aboutSection(Size size) {
    return Column(
      children: [
        Padding(
          padding:
              const EdgeInsets.only(left: 15, right: 15, top: 20, bottom: 14),
  ...
  Widget bottomBooksection(Size size) {
    return Positioned(
      bottom: 0.0,
      child: Container(
        alignment: defaultTargetPlatform == TargetPlatform.iOS
            ? Alignment.topCenter
            : Alignment.center,
        height: 10.h,
                ...
                children: [
                  data["isFree"]
                      ...
                      : Column(
                          ...
                          children: [
                            buildText("\$${data["eventFee"]}", 20,
                                FontWeight.w600, color.txtBlack),
                            Flexible(
                                child: buildText("Join our event", 16,
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: buildText(
                          data["isFree"] ? "Book now" : "Subscribe",
                          18,
                          FontWeight.w600,
                          color.txtWhite),
                    ),
  ...
  void bottomSHeet() {
    ...
    if (!data["isFree"] || data["hasPassword"]) {
      customDialogBoxTextField(
          ...
          img: AssetsPics.lockImg,
          onchng: (val) {cntl.text = val;},
          cntroller: cntl,
          onTapbutton: () {
