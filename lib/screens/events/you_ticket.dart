...
class _EventYourTicketScreenState extends State<EventYourTicketScreen> {
  ...
  Duration? myDuration;
  Duration? myDurationday;
  Duration? myDurationhour;
  Duration? myDurationmin;
  Duration? myDurationsec;
  ...
  setState(() {final seconds = myDuration!.inSeconds - reduceSecondsBy;
  if (seconds < 0) {countdownTimer!.cancel();} else {myDuration = Duration(seconds: seconds);}});}
  void startTimer() {countdownTimer = Timer.periodic(Duration(seconds: 1), (_) => setCountDown());}
  ...
  void setdateFormate(int duration){
    ...
    int hour1 = hours%24;
    startTimer();
    if (sec < 60) {myDuration=Duration(seconds:sec);}
    else if (min < 60) {myDuration=Duration(minutes: min,seconds: sec1);}
    else if (hours <= 24) {myDuration=Duration(hours:hours,minutes: min1,seconds: sec1);}
    else {myDuration=Duration(days:days,hours:hour1,minutes: min1,seconds: sec1);}
  }
                    ...
                    children: [
                      ...
                      child: Row(
                        ...
                        Expanded(
                          ...
                          days=="00"?SizedBox():  Container(
                              margin: const EdgeInsets.only(left: 9,right: 9),child: buildText(":", 28, FontWeight.w600, color.txtBlack)),
                        Column(
                          children: [
                            buildText(int.parse(hours)<0?"00":hours, 28, FontWeight.w600, color.txtBlack),
                            buildText("Hours", 16, FontWeight.w400, color.txtgrey,fontFamily: FontFamily.hellix),
                          ],),
                        Container(
                        ...
                        Column(
                          children: [
                            buildText(int.parse(minutes)<0?"00":minutes, 28, FontWeight.w600, color.txtBlack),
                            buildText("Minutes", 16, FontWeight.w400, color.txtgrey,fontFamily: FontFamily.hellix),
                          ],),
                          days!="00"?SizedBox():Container(
                              margin: const EdgeInsets.only(left: 8,right: 8),
                              child: buildText(":", 28, FontWeight.w600, color.txtBlack)),
                          days!="00"?SizedBox():  Column(
                            children: [
                              ...
                              buildText("seconds", 16, FontWeight.w400, color.txtgrey,fontFamily: FontFamily.hellix),
                            ],),
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: (){
                        ...
                        else{eventCntrller.saveEvent(ApiList.saveEvent, data["eventId"].toString());
                          LocaleHandler.items.add(data["eventId"]);}
                        Preferences.setValue("EventIds", jsonEncode(LocaleHandler.items));
                        setState(() {});
                      },
                      child: CircleAvatar(radius: 18, backgroundColor: color.txtWhite,
          ...
          Row(
            ...
            children: [
              SizedBox(
                ...
                child: buildTextOverFlow(data["title"]+" - "+data["type"], 28, FontWeight.w600, color.txtBlack),
              ),
              SizedBox(
  ...
  Column aboutSection(Size size) {
    return Column(children: [
      Padding(
        padding: const EdgeInsets.only(left: 15,right: 15,top: 80,bottom: 14),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(12),
                    gradient:  const LinearGradient(begin: Alignment.bottomCenter, end: Alignment.topCenter, colors:[color.gradientLightBlue, color.txtBlue])
                ),
