...
class _FeedPersonProfileScreenState extends State<FeedPersonProfileScreen> {
  ...
  late CachedVideoPlayerPlusController? _controller2;
  late CachedVideoPlayerPlusController? _controller3;
  cacheVideoPlay1(url){
  ...
  Future reportUser(String reason)async{
    final url = ApiList.reportUser + '${LocaleHandler.userId}/report';
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          if (dataa["profileVideos"].length != 0) {
            cacheVideoPlay1(dataa["profileVideos"][0]["key"]);
          ...
          if (dataa["profileVideos"].length >= 2) {
            cacheVideoPlay2(dataa["profileVideos"][1]["key"]);
          ...
          if (dataa["profileVideos"].length >= 3) {
            cacheVideoPlay3(dataa["profileVideos"][2]["key"]);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body:dataa==null?Center(child: CircularProgressIndicator(color: color.txtBlue)): CustomScrollView(
                      ...
                      SizedBox(
                        ...
                        child: Row(children: [
                          Expanded(child: dataa["profileVideos"].length != 0
                              ? buildVideoContainer(_controller!, )
                              : buildContainer()),
                          ...
                          return GestureDetector(
                              onTap: (){
                                customSparkBottomSheeet(context,AssetsPics.sparkleft, "Are you sure you would like to\n use 1x Spark?", "Cancel", "Yes",
                                    sparks: val.sparks,
                                    onTap2: (){Get.back();
                                    if(val.sparks<=0){customSparkBottomSheeet(context,AssetsPics.sparkempty,
                                    ...
                                    // else{Provider.of<profileController>(context,listen: false).actionForHItLike("SPARK LIKE", "000");}
