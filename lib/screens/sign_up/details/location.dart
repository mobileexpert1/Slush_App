...
// "AIzaSyD2x5SDImQ-4Suz9pHXBEFkTskYnefKkv0"
  ...
  Future<void> _getCurrentPosition() async {
    ...
    if (!hasPermission) {Fluttertoast.showToast(msg: "Location permission is neccessary");}
  ...
  Future getState()async{
    ...
    // final statess = await getAllStates();
    final statess = await getAllCities();
    for(var i=0;i<statess.length;i++){
      state.add(statess[i].name);
    }
  }
  ...
  void searchItem()async{
    if(_searchQuery==""){
      ...
      statesList = state.where((element) => element.toString().toLowerCase().startsWith(_searchQuery.toLowerCase())).toList();}}
        ...
        buildContainer(
          ...
          gesture: GestureDetector(
              child: Container(
                  ...
                  height: 20,width: 30,
                  alignment: Alignment.center,
                  child: SvgPicture.asset(AssetsPics.locationIcon))
          ),
        ),
                  ...
                  shrinkWrap: true,
                    ...
                    itemBuilder: (context,index){
                      return GestureDetector(
                        onTap: (){
                          setState(() {
                            locationController.text=statesList[index];
                            LocaleHandler.location=statesList[index];
                            _searchQuery="";
                            statesList.clear();
                          });
                        }
                        ...
                        alignment: Alignment.topLeft,
                          margin: const EdgeInsets.only(left: 10,right: 10),
                            padding: const EdgeInsets.only(left: 8,right: 8,top: 0),
                            // decoration:  BoxDecoration(border: Border(bottom: BorderSide(width: 0.5,color:index==statesList.length-1?color.txtWhite: color.txtBlue))),
                            child: buildText(statesList[index],18,FontWeight.w500,color.txtgrey)),
                      );
                    }),
              ),
            ),
