...
class _WaitingCompletedState extends State<WaitingCompleted> with SingleTickerProviderStateMixin {
  ...
  Future getFixtures() async {
    ...
    if (response.statusCode == 200) {
      Preferences.setValue("eventjoined", "yes");
      Preferences.setValue("eventID", "yes");
      Preferences.setValue("eventdateNumber", LocaleHandler.dateno.toString());
      setState(() {
...
class _WaitingCompletedFeedBackState extends State<WaitingCompletedFeedBack> with SingleTickerProviderStateMixin {
  late Timer _timer;
...
//-- TODO after terminate the app
class WaitingCompletedTerminate extends StatefulWidget {
  WaitingCompletedTerminate({super.key, required this.data});

  var data;

  @override
  State<WaitingCompletedTerminate> createState() =>
      _WaitingCompletedTerminateState();
}

class _WaitingCompletedTerminateState extends State<WaitingCompletedTerminate> with SingleTickerProviderStateMixin {
  late Timer _timer;
  late AnimationController _animationController;
  late Animation<double> _animation;
  final int startingMinutes = 6;
  ...
  Future getFixtures() async {
    final url = "${ApiList.fixtures}${widget.data["eventId"]}/fixtures";
    print(url);
    var uri = Uri.parse(url);
    var response = await http.get(uri, headers: {
      'Content-Type': 'application/json',
                            ...
                            return CircularPercentIndicator(
                              ...
                              center: Column(
                                children: [
                                  // const SizedBox(height: 45),
                                  Container(height: MediaQuery.of(context).size.height*0.07),
                                  // buildText2(mins < 0 ? "00:00" : formatDuration(Duration(seconds: mins)), 36, FontWeight.w700, color.txtBlack),
                                  // buildText2("minutes", 20, FontWeight.w500, color.txtBlack),
                                ],
                              ),
                              backgroundColor: Colors.grey,
